...
# make_windows updated by Mark Stevenson -- 31-8-2022
# fn to define windows of indexes to examine 
def make_windows(n_windows, n_samp_docs):
    split_list = np.array_split(range(n_samp_docs), n_windows)
    windows = list(zip([item[0] for item in split_list], [item[-1] for item in split_list]))
    return windows
...
def get_points_power(windows, window_size, rel_list):
    ...
    # x-values are midpoints between start and end of windows
    x = [w_s+1 for (w_s,w_e) in windows]
