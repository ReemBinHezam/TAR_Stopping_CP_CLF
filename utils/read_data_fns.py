# -*- coding: utf-8 -*-
"""
Author: Alison Sneyd / Mark Stevenson
...
# Updated version that uses two level dict structure to store lists of relevant documents
# and is significantly faster than previous version (based on lists), particualrly for
# queries with large numbers of documents 
def make_rel_dic(qrels_data):
    ...
    for row in qrels_data:
        ...
        if rel == 1:
          query_id = row[0]
          doc_id = row[2]
          if query_id in query_rel_dic: 
            query_rel_dic[query_id][doc_id] = 1
          else: 
            query_rel_dic[query_id] = {doc_id: 1}
